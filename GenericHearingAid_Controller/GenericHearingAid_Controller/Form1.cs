...
namespace GenericHearingAid_Controller
{
    ...
    {         
        ...
        chapro.CHA_DSL dsl;
        int cs = 128;
        string WorkingDirectory = @"C:\Tympan\WDRC_8BandIIR_wBT_wAFC";
        string Filename = "";
        string AFCStatus = "On";

        int afc = 0;        // 0 --> off, 1 --> on
        double mu = 0.001;   // step size
        double rho = 0.90;    // forgetting factor
        double eps = 0.008;   // power threshold
        ...
        {            
            if (applyValues) // read UI
            {
                ...
                AFCStatus = AFCcomboBox.Text.Trim();
                if (AFCStatus == "Off")
                    afc = 0;
                else afc = 1;
                AFCFilterLength = int.Parse(txtAFCFilterLength.Text);
            ...
            else // update UI
            {
                ...
                txtAFCFilterLength.Text = AFCFilterLength.ToString();
                txtEPS.Text = eps.ToString();
                txtMU.Text = mu.ToString();
                txtRHO.Text = rho.ToString();
            }
        }
        ...
        private void Directorybutton_Click(object sender, EventArgs e)
        {
            ...
            dlg.Description = "Select a folder that will be your Working Directory";
            dlg.ShowNewFolderButton = false;
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                WorkingDirectory = dlg.SelectedPath;
                txtWorkingDirectory.Text = WorkingDirectory;
                toolStripStatusLabel1.Text = "Working Directory is " + WorkingDirectory;
            }
        }
